    ...
    for i in range(size):
        for j in range(size):
            if max_coefficient < abs(coefficients[i][j]):
                ...
                max_row = i
                max_col = j
    free_members = swap_array(free_members, r, max_row)
    for l in range(size):
        coefficients = swap_matrix_row(coefficients, r, max_row, l)
    argument_positions = swap_argument_positions(argument_positions, r, max_col)
    for m in range(size):
        if m < r:
            result_coefficients = swap_matrix_columns(result_coefficients, m, r, max_col)
        else:
            coefficients = swap_matrix_columns(coefficients, m, r, max_col)
    return coefficients, free_members, argument_positions, result_coefficients


def swap_matrix_columns(matrix, r, fc, sc):
    temp = matrix[r][fc]
...
def swap_array(free_members, fc, sc):
...
def swap_matrix_row(matrix, fc, sc, col):
    temp = matrix[fc][col]
...
if __name__ == '__main__':
    argument_vector=newton_jakobi()
    print(f'{argument_vector}')
